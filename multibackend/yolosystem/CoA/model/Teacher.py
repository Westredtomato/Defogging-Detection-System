...
from data.data_loader import RESIDE_Dataset  # 确保导入正确
import math
...
class Teacher(nn.Module):
    def __init__(self, res_blocks=18):
        ...
        #源代码 res2net101.load_state_dict(torch.load('./model/imagenet_model/res2net101_v1b_26w_4s-0812c246.pth'))
        # ============== 修改开始 ==============
        # 1. 定义模型绝对路径（直接使用你的实际路径）
        # weight_path = r"C:\Users\xy-xie\CoA\model\imagenet_model\res2net101_v1b_26w_4s-0812c246.pth"
        weight_path = "/data/run01/sczd119/CoA/model/imagenet_model/res2net101_v1b_26w_4s-0812c246.pth"
        # 2. 添加路径验证和调试信息
        print("\n=== 模型路径验证 ===")
        print(f"绝对路径: {weight_path}")
        print(f"文件是否存在: {os.path.exists(weight_path)}")

        if not os.path.exists(weight_path):
            raise FileNotFoundError(
                f"模型文件未找到！请检查：\n"
                f"1. 路径是否完全一致（包括大小写和符号）\n"
                f"2. 文件是否完整（右键属性查看大小应为~130MB）\n"
                f"3. 文件权限是否允许读取"
            )

        # 3. 加载模型（添加异常捕获）
        try:
            # res2net101.load_state_dict(torch.load(weight_path))
            res2net101.load_state_dict(torch.load(weight_path, map_location=torch.device('cpu')))
            print("✅ 模型加载成功！")
...
if __name__ == "__main__":
    # !!! 修改点1：修正数据集路径（适配你的HAZE4K结构）
    # train_dir_1 = r'C:\Users\xy-xie\CoA\dataset\Haze4K\train'
    train_dir_1 = '/data/run01/sczd119/CoA/dataset/Haze4K/train'  # Linux路径示例
    print(f"数据集路径检查: {os.path.abspath(train_dir_1)}")  # 打印绝对路径

    # !!! 修改点2：验证路径是否存在
    if not os.path.exists(train_dir_1):
        raise FileNotFoundError(f"数据集路径不存在: {train_dir_1}。请检查：\n"
                                f"1. 确认dataset文件夹在项目根目录下\n"
                                f"2. 确认HAZE4K/train结构包含hazy和clear子目录")
        # 初始化数据集
    train_set_1 = RESIDE_Dataset(
        path=train_dir_1,
        train=True,
        size=256,
        format='.png'  # 根据实际文件格式修改
    )
    # 验证DataLoader
    loader = DataLoader(train_set_1, batch_size=8, shuffle=True, num_workers=4)
    batch = next(iter(loader))
    print(batch[0].shape, batch[1].shape)  # 应输出 torch.Size([8, 3, 256, 256])
        # 测试数据加载
    sample_hazy, sample_clear = train_set_1[0]
    print(f"测试加载成功！雾图类型: {type(sample_hazy)}, 清晰图类型: {type(sample_clear)}")
    print(f"雾图内容示例: {sample_hazy[0].shape if hasattr(sample_hazy, '__len__') else 'N/A'}")
