...
if __name__ == "__main__":
    ...
    train_dir_1 = './dataset/Haze4K/train'#'./data/THaze/train'
    train_set_1 = RESIDE_Dataset(train_dir_1, True, 256, '.png')# train_set_1 = RESIDE_Dataset(train_dir_1, True, 256, '.jpg')

    test_dir ='./dataset/Haze4K/test' #'./data/THaze/test'
    ...
    dataset=train_set_1,
    batch_size=24,  # 从24增大到48（根据GPU显存调整）
    shuffle=True,
    num_workers=8,           # 保持或增加（建议≤CPU核心数）
    pin_memory=True,         # 加速数据到GPU的传输
    persistent_workers=True  # 避免重复初始化worker
)
    ...
    #同步调整学习率
    # 原batch_size=24时lr=opt.start_lr
    # 新batch_size=48时，lr应加倍：
    optimizer = optim.Adam(
    params=filter(lambda x: x.requires_grad, teacher_net.parameters()),
    lr=opt.start_lr,  # 学习率按比例增大
    betas=(0.9, 0.999),
    eps=1e-08)
    optimizer.zero_grad()
