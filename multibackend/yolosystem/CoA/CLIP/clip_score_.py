...
class Prompts(nn.Module):
	def __init__(self,initials=None):
		...
		if initials!=None:
			...
			# create a parameter on the selected device and init it
			self.text_features = nn.Parameter(torch.empty(2, 512, device=torch.device(device), dtype=torch.float))
			torch.nn.init.xavier_normal_(self.text_features)
...
learn_prompt = Prompts().to(torch.device(device))
clip_normalizer = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
img_resize = transforms.Resize((224,224))
